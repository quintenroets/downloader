...
class Downloader:
    def __init__(
            self, url, dest=None, folder=None, session=None, headers=None, 
            progress_callback=None, timeout=10, skip_same_size=False
        ):
        self.url = url
    ...
    def download_modified(self, stream):        
        ...
        skip = self.skip_same_size and self.dest.size == int(stream.headers['Content-Length'])
        if not skip:
